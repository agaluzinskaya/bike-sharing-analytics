...
Based on the correlation matrix of numeric variables, humidity, casual, and registered show a strong enough coorelation with count. The variable humidity, has a weak negative relationship with count. While casual and registered both have a strong linear relationship with count. According to the p-values in the linear regression model, the predictor variables humidity, casual, and registered all have highly significant p-values < 0.05. With an adjusted $R^2$ of 1, which indicates perfect fit and that all variance has been explained. 
...
The residuals vs. fitted plot above the plot it not distributed vertically, and there are visible outliers. Since we cannot detect a clear pattern or trend, the linear model has some room for improvement. 
